#IMPLEMENTATION: 
# replace this file with the original utils file you already have in the following path:
#"C:\Users\'user'\anaconda3\envs\tudat-space\Lib\site-packages\pymsis\utils.py"
#so delete the one that is already in this path and add this one in exactly that path with the same name obv
#then make sure to change the path to the SW-all file with the values in it
...
#CHANGE THIS TO WHERE YOU KEEP THE SW FILE PATH THING
Path(__file__).parent / "SW-All (1).csv" #CHANGE THIS PATH!!!!
_DATA: dict[str, npt.NDArray] | None = None
...
def get_f107_ap(dates: npt.ArrayLike) -> tuple[npt.NDArray, npt.NDArray, npt.NDArray]:
    ...
    # Clip indices to stay within bounds for future dates newly added
    if np.any((ap_indices < 0) | (ap_indices >= len(data["ap"]))):
        warnings.warn(
            "Requested dates are outside the available geomagnetic data range. "
            f"Using last available data (from {data_end}) for future dates."
        )
    ap_indices = np.clip(ap_indices, 0, len(data["ap"]) - 1)
    daily_indices = np.clip(daily_indices, 0, len(data["f107"]) - 1)

    #USE THIS IF YOU WANT TO GO BEYOND THEIR GIVEN MAX DATE 
  #  if np.any((ap_indices < 0) | (ap_indices >= len(data["ap"]))):
  #      warnings.warn(
   #         "Using last ap value given for future thingies "
     #        )
  #  ap_indices = np.clip(ap_indices, 0, len(data["ap"]) - 1)
   # daily_indices = np.clip(daily_indices, 0, len(data["f107"]) - 1)
    
    #USE THIS FOR PAST DATES
    # if np.any((ap_indices < 0) | (ap_indices >= len(data["ap"]))):
